...
func NewServerChecker(host, user string, keyPath string, timeout time.Duration, attempts int) (*ServerChecker, error) {
...
func CheckServers(ctx context.Context, servers []*types.Server) ([]ServerResult, error) {
	var wg sync.WaitGroup
	results := make([]ServerResult, len(servers))
	for i, server := range servers {
		wg.Add(1)
		serverIP := server.Status.PublicNet.IPv4.IP
		fmt.Printf("Server %s: Labels: %+v\n", server.ObjectMeta.Name, server.ObjectMeta.Labels)
		serverPrivateKeyPath := filepath.Join(os.Getenv("HOME"), config.DefaultConfigDir, config.KeysDir, strings.Join([]string{server.ObjectMeta.Labels["lab_name"], "admin"}, "-"))
		go func(i int, server *types.Server) {
			defer wg.Done()
			sc, err := NewServerChecker(serverIP+":22", "ansible", serverPrivateKeyPath, 30*time.Minute, 20)
			if err != nil {
...
func (sc *ServerChecker) checkServerReady(ctx context.Context) error {
	...
	for {
		select {
		case <-ctx.Done():
			sc.logger.Info("Check cancelled by context",
				"host", sc.host,
				"error", ctx.Err())
			return ctx.Err()
		case <-timeout:
		...
		case <-ticker.C:
			attempt++
			sc.logger.Info("Starting connection attempt",
				"host", sc.host,
				"attempt", attempt,
				"maxAttempts", sc.attempts)
			...
			if err != nil {
				sc.logger.Error("SSH connection failed",
					"host", sc.host,
					"attempt", attempt,
					"error", err,
			...
			if err != nil {
				sc.logger.Error("Cloud-init check failed", "host", sc.host, "attempt", attempt, "error", err)
				continue
			}
			if !strings.Contains(cloudInitStatus, "done") {
				sc.logger.Info("Cloud-init not done yet", "host", sc.host, "attempt", attempt, "status", strings.TrimSpace(cloudInitStatus))
				continue
			}
			...
			if err != nil {
				sc.logger.Error("Uptime check failed", "host", sc.host, "attempt", attempt, "error", err)
				continue
			}

			bootTime, err := time.Parse("2006-01-02 15:04:05", strings.TrimSpace(uptime))
			if err != nil {
				sc.logger.Error("Failed to parse boot time", "host", sc.host, "attempt", attempt, "error", err)
				continue
			}

			if time.Since(bootTime) > 5*time.Minute {
				sc.logger.Info("Server boot time too old", "host", sc.host, "attempt", attempt)
				continue
			}
			...
			if err != nil {
				sc.logger.Error("Reboot check failed", "host", sc.host, "attempt", attempt, "error", err)
				continue
			}
			if strings.TrimSpace(rebootRequired) == "yes" {
				sc.logger.Info("Reboot still required", "host", sc.host, "attempt", attempt)
				continue
			}
			...
			if err != nil {
				sc.logger.Error("Package status check failed", "host", sc.host, "attempt", attempt, "error", err)
				continue
			}
			if strings.TrimSpace(pkgStatus) != "ok" {
				sc.logger.Info("Packages still need upgrading", "host", sc.host, "attempt", attempt)
				continue
			}
