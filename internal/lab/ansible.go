...
func (m *ManagerSvc) CreateAnsibleInventoryFile(lab *types.Lab) error {
	...
	ansibleInventoryFile := filepath.Join(homeDir, config.DefaultConfigDir, config.DefaultAnsibleDir, strings.Join([]string{lab.Name, "inventory.json"}, "-"))
	...
	for _, server := range lab.Status.Servers {
		...
		m.Logger.Debug("Adding server to inventory",
			...
			"cloud name", server.Name)
		if strings.HasSuffix(server.Name, "cp") {
...
func (m *ManagerSvc) RunAnsiblePlaybook(lab *types.Lab) error {
	...
	err = m.Storage.Save(lab)
	if err != nil {
		return fmt.Errorf("error saving lab %s: %w", lab.Name, err)
	}
